...
if resolution >= 1 % polarization correlation length exceeded, random SOP in new segment
    ...
    rng(p.seeds.seed_beta);
    p.fibre.evolBetaTab =  single(pi*resolution*(1+rand(p.fibre.nbSegments, 1)*0.01));%single(rand()*2*pi-pi) % single((rand(p.fibre.nbSegments,1)*2*pi-pi) %single(rand(p.fibre.nbSegments,1)*2*pi-pi)*0.00001 %[single(rand()*2*pi-pi); single((rand(p.fibre.nbSegments-1,1)*2*pi-pi)*resolution)];
    rng(p.seeds.seed_gamma);
...
for n=1:p.fibre.nbSegments 
    ...
    sinRot = sin(p.fibre.evolThetaTab(n)); cosRot = cos(p.fibre.evolThetaTab(n));%sinRot = sin(0); cosRot = cos(0);%sinRot = sin(p.fibre.evolThetaTab(n)); cosRot = cos(p.fibre.evolThetaTab(n));%sinRot = sin(0); cosRot = cos(0);%sinRot = sin(p.fibre.evolThetaTab(n)); cosRot = cos(p.fibre.evolThetaTab(n)); %sinRot = sin(20); cosRot = cos(20);%
