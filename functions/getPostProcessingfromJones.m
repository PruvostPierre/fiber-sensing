...
%% Calculation of the birefringence for each segment, no decimation

% Estimated matrices
listJonesMatrices = reshape(listJonesMatrices, 4, p.rx.nbOvsReflectors,p.rx.nbDetectedCodes);
productJonesMatrices = zeros(4, p.rx.nbOvsReflectors-1, p.rx.nbDetectedCodes); %product matrix for eigenvalue calculation
...
for codeIdx = 1:p.rx.nbDetectedCodes
    ...
    for segIdx = 1:p.rx.nbOvsReflectors-1
        ...
        currentJonesMatrix = currentJonesMatrix/sqrt(detCurrent);
...
for codeIdx = 1:p.rx.nbDetectedCodes
    ...
    for segIdx = 1:p.rx.nbOvsReflectors-1
        ...
        p.pola.birefringenceJonesMatrices(:, segIdx, codeIdx) =  (1/2)*getAnglePlusMinusPiOver2(angle(eigenvalues(1))) - (1/2)*getAnglePlusMinusPiOver2(angle(eigenvalues(2))); %restrict to [-pi/2, pi/2] to avoid sign ambiguity
    end
end

% Generated matrices
% Reshape p.HiGen into 2x2 matrices for each segment
p.pola.HiGen = reshape(p.pola.HiGen, 4, p.fibre.nbSegments*p.tx.nbCodes);
...
%% Calculation of the birefringence for the selected reflectors
...
% Generated matrices
p.pola.detCurrentGen = zeros(1, p.rx.nbOvsSelectedReflectors-1, p.rx.nbDetectedCodes);
productJonesMatricesGen = zeros(4, p.rx.nbOvsSelectedReflectors-1, p.rx.nbDetectedCodes);
...
p.pola.JonesMatricesGen = reshape(JonesMatricesGenSelect, 4, p.rx.nbOvsSelectedReflectors, p.rx.nbDetectedCodes); % Reshape the selected Jones matrices
p.pola.testJonesMatricesGenSelect = zeros(4, p.rx.nbOvsSelectedReflectors);
for codeIdx = 1:p.rx.nbDetectedCodes
    for segIdx = 1:p.rx.nbOvsSelectedReflectors-1
        % Extract the current and next Jones matrices
        currentJonesMatrixGen = reshape(p.pola.JonesMatricesGen(:,segIdx, codeIdx), 2, 2);
        nextJonesMatrixGen = reshape(p.pola.JonesMatricesGen(:,segIdx + 1, codeIdx), 2, 2);
        
        % Calculate the determinants
        p.pola.detCurrentGen(:, segIdx, codeIdx) = det(currentJonesMatrixGen);
        detNextGen = det(nextJonesMatrixGen);
        
        % Normalize the matrices by the square root of their determinants
        currentJonesMatrixGen = currentJonesMatrixGen / sqrt(p.pola.detCurrentGen(:, segIdx, codeIdx));
        nextJonesMatrixGen = nextJonesMatrixGen / sqrt(detNextGen);
...
% Initialize the eigenvalues matrix
p.pola.eigenvaluesJonesMatricesGen = zeros(2, p.rx.nbOvsSelectedReflectors-1, p.rx.nbDetectedCodes);
p.pola.birefringenceJonesMatricesGenSelect = zeros(1, p.rx.nbOvsSelectedReflectors-1, p.rx.nbDetectedCodes);

% Loop through each segment
for codeIdx = 1:p.rx.nbDetectedCodes
    for segIdx = 1:p.rx.nbOvsSelectedReflectors-1
        % Extract the product Jones matrix
        productMatrixGen = reshape(productJonesMatricesGen(:,segIdx, codeIdx), 2, 2);
        ...
        % Compute the birefringence
        p.pola.birefringenceJonesMatricesGenSelect(:, segIdx, codeIdx) = ...
            (1/2) * getAnglePlusMinusPiOver2(angle(eigenvaluesGen(1))) - ...
            (1/2) * getAnglePlusMinusPiOver2(angle(eigenvaluesGen(2)));
    end
end

%% standard deviation of birefringence estimation
p.pola.stdBirefringenceSelect = std(abs(p.pola.birefringenceJonesMatricesSelect(1, :, 1:end)), 0, 3);


%% Mean squared error of birefringence estimation on the selected reflectors
sum_squared = zeros(1, p.rx.nbOvsSelectedReflectors-1);
mse_persegment = zeros(1, p.rx.nbOvsSelectedReflectors-1);

for segIdx = 1:p.rx.nbOvsSelectedReflectors-1
    for codeIdx = 1:p.rx.nbDetectedCodes
        % Compute the mean squared error between the estimated and generated birefringence variations around the mean
        diff_gen = abs(p.pola.birefringenceJonesMatricesGenSelect(1, segIdx, codeIdx)) - mean(abs(p.pola.birefringenceJonesMatricesGenSelect(1, segIdx, :)));
        diff_est = abs(p.pola.birefringenceJonesMatricesSelect(1, segIdx, codeIdx)) - mean(abs(p.pola.birefringenceJonesMatricesSelect(1, segIdx, :)));
        sum_squared(segIdx) = sum_squared(segIdx) + abs((diff_gen - diff_est)).^2;
    end
    mse_persegment(segIdx) = sum_squared(segIdx)/p.rx.nbDetectedCodes; % Compute the mean squared error for each segment
end
p.pola.mseSeg = mse_persegment; % Store the mean squared error for each segment
