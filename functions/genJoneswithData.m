function [Hi,p] = genJoneswithData(p, codeIdx, data)
    %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if resolution >= 1 % polarization correlation length exceeded, random SOP in new segment
    ...
    rng(p.seeds.seed_beta);
    %p.fibre.evolBetaTab = single(2*pi*resolution*(1 + single(rand(p.fibre.nbSegments,1)*2*pi-pi)*0.00001)); %single(rand()*2*pi-pi) % single((rand(p.fibre.nbSegments,1)*2*pi-pi) %single(rand(p.fibre.nbSegments,1)*2*pi-pi)*0.00001
    p.fibre.evolBetaTab = repelem(data(:, data_idx), 4);
    disp(size(p.fibre.evolBetaTab));
    p.fibre.evolBetaTab = p.fibre.evolBetaTab(1:p.fibre.nbSegments);
    p.fibre.evolBetaTab = (p.fibre.evolBetaTab + 1) * single(2*pi*resolution); %single(rand()*2*pi-pi) % single((rand(p.fibre.nbSegments,1)*2*pi-pi) %single(rand(p.fibre.nbSegments,1)*2*pi-pi)*0.00001
    rng(p.seeds.seed_gamma);
...
for n=1:p.fibre.nbSegments 
    ...
    sinRot = sin(0); cosRot = cos(0);%sinRot = sin(p.fibre.evolThetaTab(n)); cosRot = cos(p.fibre.evolThetaTab(n)); %sinRot = sin(20); cosRot = cos(20);%
