...
switch (p.ProbingMode)
    ...
    case 'SIMO'
         intJonesTab =  p.HiTab(1,1:nbNetUpJonesMat).*conj(p.HiTab(1,1:nbNetUpJonesMat))+p.HiTab(2,1:nbNetUpJonesMat).*conj(p.HiTab(2,1:nbNetUpJonesMat));
         
    case 'MISO'
        if p.rx.Xpol %MISO X
            intJonesTab =  p.HiTab(1,1:nbNetUpJonesMat).*conj(p.HiTab(1,1:nbNetUpJonesMat))+p.HiTab(3,1:nbNetUpJonesMat).*conj(p.HiTab(3,1:nbNetUpJonesMat));
        else %MISO Y
            intJonesTab = p.HiTab(2,1:nbNetUpJonesMat).*conj(p.HiTab(2,1:nbNetUpJonesMat))+p.HiTab(4,1:nbNetUpJonesMat).*conj(p.HiTab(4,1:nbNetUpJonesMat));
        end
        
    case 'SISO' %XX ou YY - either RXX, RXY or RYY, RYX are null
        if p.rx.Xpol && p.tx.Xpol %SISO XX
            intJonesTab =  p.HiTab(1,1:nbNetUpJonesMat).*conj(p.HiTab(1,1:nbNetUpJonesMat));
        elseif p.rx.Ypol && p.tx.Ypol %SISO YY
            intJonesTab = p.HiTab(4,1:nbNetUpJonesMat).*conj(p.HiTab(4,1:nbNetUpJonesMat));
        elseif p.rx.Ypol && p.tx.Xpol %SISO XY
            intJonesTab = p.HiTab(2,1:nbNetUpJonesMat).*conj(p.HiTab(2,1:nbNetUpJonesMat));
        else %SISO YX
            intJonesTab = p.HiTab(3,1:nbNetUpJonesMat).*conj(p.HiTab(3,1:nbNetUpJonesMat));
        end
