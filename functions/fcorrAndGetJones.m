...
codeLen  = length(gCode);
netBlockLen = brutBlockLen-codeLen+1; % -codeLen +1 to avoid the last code in the block that can be impacted by time aliasing
nbFullBlocks = floor(p.rx.ErxLen/netBlockLen);
nbRemainder = mod(p.rx.ErxLen,netBlockLen);
...
Erx = ([Erx; zeros(codeLen-1,2)]);%Insert zeros at input array termination to avoid alloc error in edge calculation
...
tmpCorr = fctCorrBlock(Erx(offsetInStart:1:offsetInStart+brutBlockLen-1,:), gCode_fft, codeLen,p.rx.offset_ratio);

%netBlockLen = length(tmpCorr);
%nbFullBlocks = floor((p.rx.ErxLen-(codeLen-1))/netBlockLen);
% nbRemainder = mod(p.rx.ErxLen-(codeLen-1),netBlockLen);
...
switch(p.ProbingMode)
    ...
    case 'SIMO'
        sumAbsTmpCorr = tmpCorr(1,:).*conj(tmpCorr(1,:))+tmpCorr(2,:).*conj(tmpCorr(2,:)); 
        
    case 'MISO' % Not tested
        sumAbsTmpCorr = tmpCorr(1,:).*conj(tmpCorr(1,:))+tmpCorr(32,:).*conj(tmpCorr(3,:)); 
        
    case 'SISO' % Not tested
        sumAbsTmpCorr = tmpCorr(1,:).*conj(tmpCorr(1,:)); 
        
    otherwise
        error('Unsupported probing mode');
...
%preallocate a space for the storage of Jones matrices. Create an over-sized table
if nbRemainder<codeLen %Useless to carry out the remaining truncated block if its size < codeLen %FIXME not tested
    p.HiTab = single(NaN(4,nbFullBlocks*p.rx.nbOvsReflectors*(nbExpectedCodes+1)));
else
    p.HiTab = single(NaN(4,(nbFullBlocks+1)*p.rx.nbOvsReflectors*(nbExpectedCodes+1)));
end
...
%     end
    ...
    tmpCorr = fctCorrBlock(Erx(offsetInStart:1:offsetInStart+brutBlockLen-1,:), gCode_fft, codeLen,p.rx.offset_ratio);
    rayleighBlockMask = getRayleighBlockMask(netBlockLen, blockIdx, xStartRay, xStopRay, ovsCodeLength);%Get the mask that localizes the series of Rayleigh Jones matrices contained in the current subblock 'tmpCorr'
    idxStart = idxStart+nbMaskElts; 
    nbMaskElts = sum(rayleighBlockMask==1);
    p.HiTab(:,idxStart:idxStart+nbMaskElts-1) = tmpCorr(:,rayleighBlockMask==1);%Extract all the Rayleigh Jones matrices from the current block and store them in p.HiTab
end
clear gCode_fft; % The variable is cleared because it will be redefined in the upcoming part to process the remainder samples (shorter than a full block)

%Process the remaining block (size<netBlockLen)
if nbRemainder>=codeLen %Useless to carry out the remaining truncated block if its size < codeLen %FIXME not tested

    gCode_fft(:,1) = fft(gCode(1,:).',nbRemainder);%code spectrum zero-padded to reach length of processed block
    gCode_fft(:,2) = fft(gCode(2,:).',nbRemainder);
    offsetInStart = offsetInStart + netBlockLen; %input block translation is netBlockLen
    clear tmpCorr; 
...
% end  
    ...
    tmpCorr = fctCorrBlock(Erx(offsetInStart:1:offsetInStart+nbRemainder-1,:), gCode_fft, codeLen,p.rx.offset_ratio);
    blockIdx = nbFullBlocks + 1;
    rayleighBlockMask = getRayleighBlockMask(nbRemainder-codeLen, blockIdx, xStartRay, xStopRay, ovsCodeLength);%Get the mask that localizes the series of Rayleigh Jones matrices contained in the current subblock 'tmpCorr'
    idxStart = idxStart+nbMaskElts; 
    nbMaskElts = sum(rayleighBlockMask==1);
    p.HiTab(:,idxStart:idxStart+nbMaskElts-1) = tmpCorr(:,rayleighBlockMask==1);%Extract all the Rayleigh Jones matrices from the current block and store them in p.HiTab
